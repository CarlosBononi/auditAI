...
def aplicar_estilo_pericial(texto):
    ...
    # A cor agora é definida estritamente pela linha de classificação
    if "CLASSIFICACAO: SEGURO" in texto_upper: cor = "#27ae60" # VERDE
    elif "FRAUDE CONFIRMADA" in texto_upper: cor = "#c0392b" # VERMELHO
    elif "POSSIVEL FRAUDE" in texto_upper: cor = "#d35400" # LARANJA
    elif "ATENCAO" in texto_upper or "IA" in texto_upper: cor = "#f1c40f" # AMARELO
    else: cor = "#2980b9" # AZUL (NEUTRO)
...
# 7. MOTOR PERICIAL COM ANÁLISE SEGREGADA (FIX DE ASSOCIAÇÃO)
        ...
        if not user_query and not st.session_state.arquivos_acumulados:
            ...
            with st.spinner("🕵️ Realizando auditoria segregada..."):
                try:
                    ...
                    tem_email = any(f["name"].endswith(".eml") for f in st.session_state.arquivos_acumulados)

                    # INSTRUÇÕES SEGREGADAS PARA EVITAR MIX-UP
                    instrucao = f"Aja como AuditIA sênior. Hoje: {agora}. Regras Gerais: 1.Inicie CLASSIFICACAO: [TIPO]. 2.Seja técnico.\n"
                    if tem_imagem:
                        instrucao += "PROTOCOLO IMAGEM: Analise mãos, olhos, física da luz e texturas. Se não houver metadados EXIF reais, use 'CLASSIFICACAO: ATENCAO (POSSIVEL IA)'. Proibido dizer 'Autêntica' para imagens sem EXIF.\n"
                    if tem_email:
                        instrucao += "PROTOCOLO E-MAIL: Analise SPF, DKIM, cabeçalhos ARC e display name spoofing. Ignore anatomia e física da luz.\n"
                    
                    contexto = [instrucao]
                    for f in st.session_state.arquivos_acumulados:
                        if f["name"].endswith(".eml"): contexto.append(f"E-MAIL RAW: {f['content'].decode('utf-8', errors='ignore')[:2000]}")
                    ...
                    # RETENTATIVA PARA ERRO 429
                    for attempt in range(3):
                        try:
                            res = model.generate_content(contexto, request_options={"timeout": 150})
                            st.session_state.historico_pericial.append(res.text)
                            st.rerun(); break
                        except Exception as e:
                            if "429" in str(e): time.sleep(5 * (attempt + 1))
                            else: raise e
...
st.caption(f"AuditIA © {datetime.now().year} - Vargem Grande do Sul - SP | Versão 2.1 Rigor Forense")
